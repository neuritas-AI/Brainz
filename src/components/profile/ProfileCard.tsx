...
export default function ProfileCard({ session }: { session: any }) {
  ...
  async function saveProfile(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || t.unableUpdateProfile);
      }
      ...
      setMessage(error instanceof Error ? error.message : t.unableUpdateProfile);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-glow">
      ...
      <form onSubmit={saveProfile} className="mt-4 space-y-4">
        ...
        <label className="block text-sm text-slate-200">{t.displayName}
          <input className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3" value={profileName} onChange={(e) => setProfileName(e.target.value)} placeholder={t.displayName} />
        </label>
        <label className="block text-sm text-slate-200">{t.newPassword}
          <input className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t.newPassword} />
        </label>
        <button disabled={loading} className="w-full rounded-2xl bg-gradient-to-r from-brand-blue to-brand-purple px-4 py-3 font-semibold disabled:opacity-60">{loading ? t.saving : t.saveProfile}</button>
